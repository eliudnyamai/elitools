          ...
          $desired_dir="uploads"; //specify the desired directory name here 
              if(is_dir("$desired_dir/".$file_name)==false){ //check if the file already exists in the desired directory or not. If not then move it. 
                  if(move_uploaded_file($file_tmp,"$desired_dir/".$file_name)){
                    ...
                    $data["success"]=true;
                    $data["message"]="<a href='php/$desired_dir/$filenameminusextension.$image_format'  download><button  id='download-btn' class='btn btn-primary'>Download $filenameminusextension.$image_format</button></a>";
                    echo json_encode($data);
                    exit();
                }
                  else{
